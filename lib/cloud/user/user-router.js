...
function initRouter(userOperations) {
  ...
  router.route('/').get(function(req, res, next) {
    console.time('DEBUG-getUsersList');
    userOperations.list().then(function(users) {
      console.timeEnd('DEBUG-getUsersList');
      res.json(users);
    }, function(error) {
      console.timeEnd('DEBUG-getUsersList');
      next(error);
  ...
  router.route('/:id').get(function(req, res, next) {
    console.time('DEBUG-readUserById');
    userOperations.read(req.params.id).then(function(user) {
      console.timeEnd('DEBUG-readUserById');
      res.json(user);
    }, function(error) {
      console.timeEnd('DEBUG-readUserById');
      next(error);
  ...
  router.route('/:id').put(function(req, res, next) {
    console.time('DEBUG-updateUserById');
    userOperations.update(req.body).then(function(user) {
      console.timeEnd('DEBUG-updateUserById');
      res.json(user);
    }, function(error) {
      console.timeEnd('DEBUG-updateUserById');
      next(error);
